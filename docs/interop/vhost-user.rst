...
Backends should always implement the ``VHOST_USER_PROTOCOL_F_MQ`` protocol
...
The backend relies on the backend communication channel (see :ref:`Backend
...
.. Note::
   Backends that report ``VHOST_USER_F_PROTOCOL_FEATURES`` must
...
.. Note::
   Backends that report ``VHOST_USER_F_PROTOCOL_FEATURES`` must support
